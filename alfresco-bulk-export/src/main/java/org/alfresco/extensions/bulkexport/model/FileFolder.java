...
public class FileFolder {
	...
	/**
	 * {@link String} path to export data location in Alfresco server
	 */
	private String basePath;

	/**
	 * {@link Boolean} value to avaliate if ovewrite content exported or no
	 */
	 ...
	 * @param basePath
	 */
	public FileFolder(String basePath, String aspects, String properties, String updateTypesOrAspectsOrProperties,
			String updateModelPrefix, boolean scapeExported, ServiceRegistry serviceRegistry,
			StringBuilder parentLogger) {
	 ...
	 * @throws Exception
	 */
	public void insertFileProperties(NodeRef nodeRef, String type, List<String> aspects, Map<String, String> properties,
			String filePath, String revision) throws Exception {
		filePath = this.basePath + filePath;
		...
		if (null == revision) {
			...
			if (this.isFileExist(filePath) && this.isFileExist(filePath + ".metadata.properties.xml" + ".v" + revision)
					&& this.scapeExported) {
				return;
			}
		}
		...
		// ZIA - START
		/*
		 * Alfresco 4.x has a bug that gets empty date when getting all versions. It
		 * works as expected when we export only the head version. The method below
		 * stores all date properties and checks before writing to xml
		 */
		this.setInvalidTypeProperties(nodeRef, type, aspects);

		/*
		 * We are not going to check the attributes of the custom aspects, because they
		 * might not exist in the current system. We only want to add the aspects and
		 * properties to be used in the importing system
		 */
		...
		if (null != properties && (null != this.customProperties && !this.customProperties.isEmpty())) {
			for (Entry<String, String> prop : this.customProperties.entrySet()) {
				/*
				 * If we want to get the value from an existing property use property like
				 * "my:customProperty=cm:name"
				 */
		...
		/*
		 * Update type names, aspect names and property names. The input should be like
		 * the following my:aspectName=myapp:aspectName;my:oldProp=myapp:newProp
		 */
...
//            out.close();
	...
	private Map<String, String> getUpdatedPrefixProperties(Map<String, String> properties,
			Entry<String, String> prefixSet) {
		Map<String, String> newProperties = new HashMap<String, String>();
		for (Entry<String, String> property : properties.entrySet()) {
			if (property.getKey().startsWith(prefixSet.getKey())) {
				newProperties.put(property.getKey().replaceFirst(prefixSet.getKey(), prefixSet.getValue()),
						property.getValue());
			} else {
	...
	private void setInvalidTypeProperties(NodeRef nodeRef, String type, List<String> aspects) {
		...
		if (null != type && !type.isEmpty() && !this.processedTypesAndAspects.contains(type)) {
			props.putAll(
					serviceRegistry.getDictionaryService().getType(getQNameFromPrefixString(type)).getProperties());
		...
		if (null != aspects && !aspects.isEmpty() && !this.processedTypesAndAspects.containsAll(aspects)) {
			for (String aspect : aspects) {
				...
				props.putAll(serviceRegistry.getDictionaryService().getAspect(getQNameFromPrefixString(aspect))
						.getProperties());
