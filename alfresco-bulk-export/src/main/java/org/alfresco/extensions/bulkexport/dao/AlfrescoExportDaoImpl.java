...
public class AlfrescoExportDaoImpl implements AlfrescoExportDao {
	...
	private QName ignorePropertyQname[] = { ContentModel.PROP_NODE_DBID, ContentModel.PROP_NODE_UUID,
			ContentModel.PROP_CATEGORIES, ContentModel.PROP_CONTENT, ContentModel.ASPECT_TAGGABLE,
	...
	private QName[] ignoredType = { ContentModel.TYPE_SYSTEM_FOLDER, ContentModel.TYPE_LINK, ContentModel.TYPE_RATING,
			ActionModel.TYPE_ACTION, ActionModel.TYPE_COMPOSITE_ACTION, PublishingModel.TYPE_PUBLISHING_QUEUE };

	private List<QName> ignoredAspects = Collections.unmodifiableList(new ArrayList<QName>() {
		{
	...
	public Map<String, String> getPropertiesAsString(NodeRef nodeRef) throws Exception {
		...
		for (QName qName : qNameSet) {
			...
			// put key value in the property list as <prefixOfProperty:nameOfProperty,
			// valueOfProperty>
	...
	/**
	 * @see com.alfresco.bulkexport.dao.AlfrescoExportDao#getProperty(java.lang.String,
	 *      java.lang.String)
	 */
	...
	public Map<String, NodeRefRevision> getNodeRefHistory(String nodeRef) throws Exception {
		...
		if (nr != null) {
			...
			while (iterator.hasNext()) {
				...
				// this contains a list of all attributes for the Item. We may not need it since
				// we dig them out at a store item id level.
				...
				//
				// we need to get the comment history as well because this is not available when
				// we get content data and properties....
