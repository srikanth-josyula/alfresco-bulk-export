 ...
 */
/**
 * Updated the class to handle injecting aspects, custom properties, export by
 * from and to dates and updating model prefixes.
 * 
...
public class Export extends DeclarativeWebScript {
	 ...
	 * @throws IOException
	 */
	// public Map<String, Object> executeImpl(WebScriptRequest req,
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
		...
		if (req.getParameterNames().length == 0
				|| (req.getParameterNames().length == 1 && null != req.getParameter("format"))) {
		...
		// If set to true then read a node.cache in the export directory as opposed to
		// rescanning for nodes to export.
		//
		...
		fileFolder = new FileFolder(base, aspects, properties, updateTypesOrAspectsOrProperties, updateModelPrefix,
				scapeExported, serviceRegistry, sb);
		...
		try {
			...
			/*
			 * ExecutorService pool = Executors.newCachedThreadPool(); Callable<Boolean>
			 * callable = new ExportCallable(engine, nf); Future<Boolean> future =
	...
	public Map<String, Object> updateModel(Map<String, Object> model) {
		 ...
		 * model.put("totalNodesToExport",
		 * runningExports.values().stream().findFirst().get().getTotalNodesToExport());
		 * model.put("availableNodesToExport",
		 * runningExports.values().stream().findFirst().get().getAvailableNodesToExport(
		 * )); model.put("previouslyExportedNodes",
		 * runningExports.values().stream().findFirst().get().getPreviouslyExportedNodes
		 * ());
		 */
		return model;
	}
	/*
	 * public static class ExportCallable implements Callable<Boolean> { private
	 * NodeRef nodeRef = null; private Engine engine = null; public
	 * ExportCallable(Engine engine, NodeRef nodeRef) { this.engine = engine;
	 * this.nodeRef = nodeRef; } public Boolean call() throws Exception { Boolean
	 * status = AuthenticationUtil.runAs(new AuthenticationUtil.RunAsWork<Boolean>()
	 * { public Boolean doWork() throws Exception { return engine.execute(nodeRef);
	 * } }, AuthenticationUtil.getSystemUserName()); return status; } }
	 */
}
